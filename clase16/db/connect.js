...
const { DB_HOST, DB_PORT, DB_NAME } = require('./config.json');
const connectionUrl = `mongodb://${DB_HOST}:${DB_PORT}`;

module.exports = ( ()=>{
    let instance = null,
    isDiscconnecting = false;
    ...
    function disconnect(){
        if(instance && !isDiscconnecting){
            isDiscconnecting = true;
            console.log('Desconectando instancia de Mongo');
            return new Promise( (resolve, reject)=>{
                instance.close( (err, result)=>{
                    if(err) { reject(err); isDiscconnecting=false;return; }
